	     ...
	     loff_t *f_pos)
{
  ...
  unsigned long j;
  unsigned long time_cond;
  unsigned long remaining_jiffies;

  dev->flag = 0;
  char temp_buf[count];
  unsigned long l = copy_from_user(temp_buf, buf, count);
  //printk("num bytes that could not be copied = %lu \n", l);
  if(l != 0) 
    {
      mutex_unlock(&dev->sleepy_mutex);
      return -EINVAL;
    }
  int time_val = *((int*)temp_buf);
  j = jiffies;
  time_cond =time_val*HZ; //time_val seconds (expressed in jiffies) into future
  // printk("jiffies = %ld : time_cond_jiffy = %ld", j, time_cond);
  mutex_unlock(&dev->sleepy_mutex);
  remaining_jiffies = wait_event_interruptible_timeout(dev->sleepy_queue, dev->flag != 0, time_cond);
			...
			struct class *class)
{
  ...
  init_waitqueue_head(&dev->sleepy_queue);
  return 0;
}
